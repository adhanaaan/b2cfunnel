...
describe("insertWithOptionalColumn", () => {
  const row = { email: "a@b.co", time_ms: 300 };

  it("includes the column when the database has it", async () => {
    const insert = vi.fn().mockResolvedValue({ error: null });
    const { error } = await insertWithOptionalColumn("tips_consent", true, row, insert);
  ...
  it("retries without the column when it is missing", async () => {
    ...
    const { error } = await insertWithOptionalColumn("tips_consent", false, row, insert);
  ...
  it("passes any other error back to the caller", async () => {
    ...
    const { error } = await insertWithOptionalColumn("tips_consent", null, row, insert);
