...
/**
 * The attract screen for /general (designed against a 1920x1080 55" panel,
 ...
 * share - GENERAL_SOURCE is what keeps NTU Homecoming's standings off this
 * screen.
 */
...
import { GENERAL_PAUSED, GENERAL_SOURCE } from "@/config/event";
...
const PLAY_URL = playUrlFor("general");
...
export default function GeneralLeaderboardBoard() {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/leaderboard?limit=${TOP_N}&source=${encodeURIComponent(GENERAL_SOURCE)}`,
          { cache: "no-store" },
        );
        ...
        if (!firstLoadRef.current && !GENERAL_PAUSED) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/report-rate?source=${encodeURIComponent(GENERAL_SOURCE)}`,
          { cache: "no-store" },
        );
        const data = await res.json();
