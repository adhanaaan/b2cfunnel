...
/**
 * The attract screen for /ihh - the /ihhsearegatta board, unchanged apart
 * from the bucket it reads and the URL its code sends players to (Figma node
 * 638:5063, designed
 ...
 * `ihh` bucket, and keeps the last good standings on error - so this board
 * ranks only /ihh players, never the regatta's.
 */
...
import { IHH_PAUSED, IHH_SOURCE } from "@/config/event";
...
/**
 * Artwork for the code, if it has been uploaded (see ScanCode). Deliberately
 * NOT the regatta's /regatta-qr.png: that file encodes /ihhsearegatta, and a
 * player who scanned it here would be scored into the regatta's bucket. Until
 * an /ihh code is uploaded under this name, the generated fallback below is
 * what the board shows, and it encodes PLAY_URL.
 */
const QR_IMAGE = "/ihh-qr.png";
...
export default function LeaderboardIhhBoard() {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/leaderboard?limit=${TOP_N}&source=${encodeURIComponent(IHH_SOURCE)}`,
          { cache: "no-store" },
        );
        ...
        if (!firstLoadRef.current && !IHH_PAUSED) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/report-rate?source=${encodeURIComponent(IHH_SOURCE)}`,
          { cache: "no-store" },
        );
        const data = await res.json();
