...
/**
 * The attract screen for /speedgame (designed against a 1920x1080 55" panel,
 * read from 2-5m away, but laid out to reflow down to a phone).
 *
 * Independent duplicate of the /event-v3/leaderboard board, wired to its own
 * SPEEDGAME_SOURCE bucket so its standings never mix with event-v3's.
 *
...
import { SPEEDGAME_PAUSED, SPEEDGAME_SOURCE } from "@/config/event";
...
export default function LeaderboardSpeedgameBoard() {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/leaderboard?limit=${TOP_N}&source=${encodeURIComponent(SPEEDGAME_SOURCE)}`,
          { cache: "no-store" },
        );
        ...
        if (!firstLoadRef.current && !SPEEDGAME_PAUSED) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/report-rate?source=${encodeURIComponent(SPEEDGAME_SOURCE)}`,
          { cache: "no-store" },
        );
        const data = await res.json();
  ...
  return (
    ...
    >
      ...
      >
        <Masthead live={!SPEEDGAME_PAUSED} />
      </header>
