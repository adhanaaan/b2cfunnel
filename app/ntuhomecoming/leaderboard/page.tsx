...
/**
 * The attract screen for /ntuhomecoming (designed against a 1920x1080 55"
...
import { NTU_HOMECOMING_PAUSED, NTU_HOMECOMING_SOURCE } from "@/config/event";
...
const PLAY_URL = playUrlFor("ntuhomecoming");
...
export default function NtuHomecomingLeaderboardBoard() {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/leaderboard?limit=${TOP_N}&source=${encodeURIComponent(NTU_HOMECOMING_SOURCE)}`,
          { cache: "no-store" },
        );
        ...
        if (!firstLoadRef.current && !NTU_HOMECOMING_PAUSED) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/report-rate?source=${encodeURIComponent(NTU_HOMECOMING_SOURCE)}`,
          { cache: "no-store" },
        );
        const data = await res.json();
  ...
  return (
    ...
    >
      ...
      >
        <Masthead live={!NTU_HOMECOMING_PAUSED} />
      </header>
