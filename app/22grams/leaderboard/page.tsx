...
/**
 * The attract screen for /22grams: the /phkl board (Figma 739:10645), pointed
 * at this event's bucket. Designed against a 1920x1080 55" panel read from
 * 2-5m away.
 *
 * The horizontal format is /phkl's, as briefed: the pitch on the left - the
 * brain, the headline, the scan block and the ember panel - and the live
 * standings on the right, over a fact strip and a band of event photography.
 * Its podium is three rows deep: the leader as the tall gradient hero, ranks 2
 * and 3 on the same gradient a row height down, ranks 4-6 on white.
 *
 * ONE THING IS NOT /phkl's, and deliberately: there is no prize on this event,
 * as there is none on the /ntuhomecoming arc it runs. The ember panel that
 * carries the Grab vouchers on the Kuala Lumpur board carries the three steps
 * of the event here instead - same box, same gradient, same weight in the
 * composition - so nothing on the screen promises something the event is not
 * giving away. Should a prize be announced, this panel is where it goes, and
 * `config/siloam.ts` is the pattern for it: the ladder in a config, the podium
 * depth read from its length, so the panel and the gradient rows cannot
 ...
 *
 * Self-contained: polls /api/leaderboard every 8s, scoped to the `22grams`
 * bucket, and keeps the last good standings on error. That bucket is the whole
 * reason this board is a route of its own rather than /ntuhomecoming's in a
 * different frame: the two events run the same arc, and only the tag on each
 * row keeps one event's times off the other's screen.
 */
...
const TOP_N = 6;
/**
 * How many rows ride the gradient. Three, as the design has it - the podium,
 * which is a podium whether or not anything is being handed out at the end of
 * it. It is also what the celebration takeover announces, so the two read the
 * same depth from one number.
 */
const PODIUM_N = 3;
const POLL_MS = 8000;
const FACT_MS = 8000;
 ...
 */
const PLAY_URL = playUrlFor("22grams");
...
/**
 * The three steps, on the ember panel rather than rotating through the fact
 * strip: with no prize to announce, the panel's job is telling the room what
 * scanning the code actually costs them, and that is worth having on screen
 * the whole time rather than one slot in eight.
 */
const HOW_TO = [
  "Play the speed game",
...
const PANEL_GRADIENT = "linear-gradient(90deg, #f77528 0%, #ff9a4d 100%)";
const STRIP_BG = "rgba(255, 255, 255, 0.72)";

/**
 * Artwork dropped in as a file under public/images/22grams/board/ (see the
 * README there). Optional: the board reads before it lands, and generates its
 * own code in the meantime rather than showing a broken image.
 */
const QR_IMAGE = "/images/22grams/board/qr.png";
 ...
 * type (the podium goes three deep); ranks 4-6 are white rows with the peach
          ...
          {leader ? (
            ...
            >
              <span
                ...
                style={{ color: LEADER_LABEL, fontSize: u(16.57) }}
              >
                Time to beat
              </span>
              <span
...
/**
 * The ember panel beside the code (739:10677's box, 775x422). On /phkl this is
 * the prize; with nothing to give away here it is the three steps, numbered,
 * at a size that reads from the back of a room - which is the one thing a
 * passer-by actually needs off this board that the standings do not already
 * say.
 */
function HowItWorksPanel() {
  return (
    <div
      className="relative flex w-full flex-col justify-center gap-[calc(var(--u)*16)] px-[calc(var(--u)*28)] py-[calc(var(--u)*28)] board:mb-[calc(var(--u)*5)] board:h-[calc(var(--u)*422)] board:w-[calc(var(--u)*775)] board:shrink-0 board:gap-[calc(var(--u)*24)] board:self-end board:px-[calc(var(--u)*41)] board:py-0"
      style={{ background: PANEL_GRADIENT, borderRadius: u(20) }}
    >
      <p className="text-[length:calc(var(--u)*16)] font-bold uppercase leading-[1.1] tracking-[0.23em] text-cream board:text-[length:calc(var(--u)*23.73)]">
        Three steps, under two minutes
      </p>

      <ol className="flex flex-col gap-[calc(var(--u)*14)] board:gap-[calc(var(--u)*22)]">
        {HOW_TO.map((step, i) => (
          <li
            key={step}
            className="flex items-center gap-[calc(var(--u)*16)] board:gap-[calc(var(--u)*24)]"
          >
            <span
              className="flex size-[calc(var(--u)*44)] shrink-0 items-center justify-center rounded-full bg-white text-[length:calc(var(--u)*24)] font-extrabold leading-none board:size-[calc(var(--u)*52)] board:text-[length:calc(var(--u)*28)]"
              style={{ color: ORANGE_DEEP }}
            >
              {i + 1}
            </span>
            <span className="min-w-0 text-[length:calc(var(--u)*24)] font-extrabold leading-[1.15] tracking-[-0.01em] text-cream board:text-[length:calc(var(--u)*38)]">
              {step}
            </span>
          </li>
        ))}
      ...
      <p
        className="text-[length:calc(var(--u)*22)] font-semibold leading-[1.3] board:text-[length:calc(var(--u)*26)]"
        style={{ color: LEADER_LABEL }}
      >
        Free, personalised, and yours to keep.
      </p>
    </div>
  );
}
 ...
 * The photos are the regatta board's, as every board on this frame uses: the
 * same three frames, one event's crowd standing in for the next. In the frame
 * the band starts 12px under the strip's bottom edge, which is why the strip
 * stacks above it.
 */
const BAND = [
...
export default function TwentyTwoGramsLeaderboardBoard() {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/leaderboard?limit=${TOP_N}&source=${encodeURIComponent(TWENTY_TWO_GRAMS_SOURCE)}`,
          { cache: "no-store" },
        );
        ...
        if (!firstLoadRef.current && !TWENTY_TWO_GRAMS_PAUSED) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/report-rate?source=${encodeURIComponent(TWENTY_TWO_GRAMS_SOURCE)}`,
          { cache: "no-store" },
        );
        const data = await res.json();
  ...
  // Strip slots: every brain fact, and - once there is one - the live
  // completion rate. The how-to is not in the rotation on this board: it is on
  // the panel, on screen the whole time.
  const slots = BRAIN_FACTS.length + (reportPct !== null ? 1 : 0);
  ...
  const showRate = reportPct !== null && slot === BRAIN_FACTS.length;
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex w-full flex-1 flex-col board:min-h-0 board:max-w-[calc(var(--u)*1920)] board:flex-row">
        ...
        <div className="flex min-w-0 flex-col px-[calc(var(--u)*24)] pt-[calc(var(--u)*40)] board:w-[calc(var(--u)*646)] board:shrink-0 board:justify-center board:pb-[calc(var(--u)*76)] board:pl-0 board:pr-[calc(var(--u)*20)] board:pt-[calc(var(--u)*46)]">
          <BoardLabel live={!TWENTY_TWO_GRAMS_PAUSED} />
          <ol
