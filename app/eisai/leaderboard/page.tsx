...
/**
 * The attract screen for /eisai (designed against a 1920x1080 55" panel,
 ...
 * share - EISAI_SOURCE is what keeps NTU Homecoming's standings off this
 * screen.
 */
...
import { EISAI_PAUSED, EISAI_SOURCE } from "@/config/event";
...
const PLAY_URL = playUrlFor("eisai");
    ...
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
      <div className="min-w-0 sm:flex-1">
        <div className="flex items-center gap-4">
          ...
          >
            World Alzheimer&apos;s Day Leaderboard
          </p>
          <p
...
export default function EisaiLeaderboardBoard() {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/leaderboard?limit=${TOP_N}&source=${encodeURIComponent(EISAI_SOURCE)}`,
          { cache: "no-store" },
        );
        ...
        if (!firstLoadRef.current && !EISAI_PAUSED) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/report-rate?source=${encodeURIComponent(EISAI_SOURCE)}`,
          { cache: "no-store" },
        );
        const data = await res.json();
