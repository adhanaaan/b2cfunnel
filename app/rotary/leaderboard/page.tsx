...
/**
 * The attract screen for /rotary (designed against a 1920x1080 55" panel,
...
import { ROTARY_CLUB_PAUSED, ROTARY_CLUB_SOURCE } from "@/config/event";
...
const PLAY_URL = playUrlFor("rotaryclub");
...
export default function RotaryClubLeaderboardBoard() {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/leaderboard?limit=${TOP_N}&source=${encodeURIComponent(ROTARY_CLUB_SOURCE)}`,
          { cache: "no-store" },
        );
        ...
        if (!firstLoadRef.current && !ROTARY_CLUB_PAUSED) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/report-rate?source=${encodeURIComponent(ROTARY_CLUB_SOURCE)}`,
          { cache: "no-store" },
        );
        const data = await res.json();
  ...
  return (
    ...
    >
      ...
      >
        <Masthead live={!ROTARY_CLUB_PAUSED} />
      </header>
