...
/**
 * Daylight shell for the /speedgame arc: an independent duplicate of
 * Event3Shell. Warm cream radial backdrop with the rotated yellow "pill"
 * lines gliding along their diagonal axis, plus optional twinkling sparkle
 * glyphs and soft ambient blobs. Fixed backdrop, same overscroll trick as
 * ScreenShell/ArenaShell/Event3Shell.
 */

interface SpeedgameShellProps {
...
export function SpeedgameShell({
  children,
  pills = true,
  sparkles = false,
  blobs = false,
  ...
  useEffect(() => {
    ...
    html.classList.add("speedgame-locked");
    document.body.classList.add("speedgame-locked");
    window.scrollTo(0, 0);
    return () => {
      html.classList.remove("speedgame-locked");
      document.body.classList.remove("speedgame-locked");
    };
  }, []);
  ...
  return (
    <main className="variant-event2 fixed inset-0 isolate flex flex-col items-center overflow-hidden px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))] text-charcoal">
      <div
        aria-hidden
        className="speedgame-daylight pointer-events-none fixed inset-0 z-0 overflow-hidden"
      >
        {blobs && (
          <>
